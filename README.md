...
```mermaid
flowchart TD
    A([Start]) --> B[/"Enter purchase amount A"/]
    B --> C{"Is A > 1000?"}

    C -- Yes --> D[discount = A × 0.10]
    D --> E[final amount = A - discount]
    E --> F[/"Display final amount"/]
    F --> G([End])

    C -- No --> H[final amount = A]
    H --> F
